namespace Problem03.ReversedList.Tests
{
    ...
    public class ListTests
    {
        ...
        public void RemoveShouldWorkCorrectly()
        {
            ...
            var randomIndex = this.random.Next(array.Length) + 1;
        ...
        public void InsertShouldThrowExceptionIfInvalidIndexIsPassed()
        {
            ...
            for (var i = 0; i < count; i++)
            {
                Assert.Throws<IndexOutOfRangeException>(() => { list.Insert(-(1 + i), this.random.Next(1, 5)); });
                Assert.Throws<IndexOutOfRangeException>(() => { list.Insert(count + i, this.random.Next(1, 5)); });
            }
        }
